...
// const yggUtils = require('yggjs-utils');
...
var dataToJsonNonArray = function (data) {
    data.toJSON = function(key) {
        var replacement = new Object();
        for (var val in this){
            if (typeof (this[val]) === 'string')
...
var bodyHashHex = function (jsonBody) {
    ...
    // return yggUtils.sha3(Buffer.from(jsonBody));
}
...
var branchId = branch => {
    return keccak(toBuffer(dataToJsonNonArray(branch))).toString('hex').substring(24,64)
};
...
module.exports = {
    ...
    dataToJsonNonArray: dataToJsonNonArray,
    toBuffer: toBuffer,
    branchId: branchId,
