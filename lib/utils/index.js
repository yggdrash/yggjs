...
const { toBuffer, isValidPrivate } = require('../local/utils')
const yggUtils = require('../local/utils');
...
var jsonToArrayString = function (data) {
...
//                         let branchId = yggUtils.sha3(branchIdByte);

//                         return branchId.toString("hex").substring(24,64);                           
// }

var bodyHashHex = function (jsonBody) {
    return yggUtils.keccak(Buffer.from(jsonBody)).toString("hex");
}
...
var branchId = branch => {
    return keccak(toBuffer(jsonToString(branch))).toString('hex').substring(24,64)
};


var fee = function (body) {
    return body.length * 5*Math.pow(10, 14);
};

var toSeed = function (value) {
    return bigInt(value * Math.pow(10, 18));
};

var fromSeed = function (value) {
    const decimal = new BigNumber("1000000000000000000");
    const bigBalance = new BigNumber(value);
    let result = bigBalance.div(decimal);
    return result;
};
...
module.exports = {
    ...
    jsonToArrayString: jsonToArrayString,
