...
const BN = yggUtil.BN
const { sha3, dataToJson } = require('../utils')
const { branchId } = require('../../utils/utils')
...
class Transaction {
  ...
  //         "params":[
  //             { 
  //             "branchId": branchId(seed.name, seed.property, seed.type, timestamp, seed.version, seed.reference_address, seed.reserve_address), 
  //             "branch": {
  //                     "name": seed.name,
  //                     "symbol": seed.symbol,
  ...
  //  }
   ...
   branchRegister (branch, signature) {
    // TODO: data exception handling 
     const stem = "fe7b7c93dd23f78e12ad42650595bc0f874c88f7"
    //  console.log(branch.params)
     const timestamp = Math.round(new Date().getTime() / 1000);
     let bodyJson = dataToJson(branch);
     const txData = {
         "timestamp": timestamp,
         "bodyLength": bodyJson.length,
         "body": bodyJson,
         "author": branch.params[0].branch.owner.substring(2),
         "hash": this.getTxHash(signature),
         "chain": stem,
         "type": branch.params[0].branch.type,
         "version": branch.params[0].branch.version,
         "bodyHash": this.bodyHashHex(branch),
         "signature": this.vrs(signature),
     }
  ...
  bodyHashHex (branch) {
    let bodyJson = dataToJson(branch);
    return sha3(Buffer.from(bodyJson)).toString("hex");
  }
