...
const BN = yggUtil.BN
const { sha3, pubToAddress } = require('../utils')
...
class Transaction {
  ...
  serialize (body, branchId) {
    ...
    const txData = {
        ...
        "body": bodyJson,
        "author": pubToAddress(this.getSenderPublicKey(), true).toString('hex').toLocaleLowerCase(),
        "hash": this.getTxHash(),
  ...
  getTxHash () {
    ...
    return sha3(Buffer.from(items, 'hex')).toString("hex");
   }
