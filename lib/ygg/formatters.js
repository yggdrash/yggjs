...
const inputAddressFormatter =  (address) => {
    ...
    if (iban.isValid() && iban.isDirect()) {
        return iban.address().substring(2);
...
const inputSeedFormatter = (seed) => {
    ...
    // const body = {
    //     "branchId": utils.branchId(seed.name, seed.property, seed.type, timestamp, seed.version, seed.reference_address, seed.reserve_address.toLocaleLowerCase().substring(2)), 
    //     "branch": {
    //             "name": seed.name,
    //             "symbol": seed.symbol,
    ...
    //             "reserve_address": seed.reserve_address.toLocaleLowerCase().substring(2),
    //             "owner": seed.owner.toLocaleLowerCase().substring(2),
    //             "timestamp": timestamp,
    ...
    // let branchId = utils.keccak(Buffer.concat([Buffer.from(seed.owner), Buffer.from(utils.decimalToHex(timestamp))])).toString('hex').substring(24,64)

    // let msg = utils.keccak(Buffer.concat([Buffer.from(seed.owner), Buffer.from(utils.decimalToHex(timestamp))]))
    
    // let signature = ksHelper.signWithPK(Buffer.from('4524b923d038405a8ad00754c507e08036df63cf28d32a4ed5f320ed0eac10d0', 'hex'), msg)

    let data = {
        'owner': `0x${seed.owner}`,
        'timeStamp': `0x${utils.decimalToHex(timestamp)}`
    }

    let transaction = new tx(data)
    transaction.sign(Buffer.from('4524b923d038405a8ad00754c507e08036df63cf28d32a4ed5f320ed0eac10d0', 'hex'))
    ...
    const branch = {
        ...
        "signature": transaction.vrs()
    }

    return branch;
}

const inputBranchIdFormatter = (seed) => {
    branchId = utils.branchId(seed.name, seed.property, seed.type, timestamp, seed.version, seed.reference_address, seed.reserve_address.toLocaleLowerCase().substring(2))
    return branchId
}
