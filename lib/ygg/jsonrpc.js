...
Jsonrpc.sendAdminTX = function (method, params, host) {
    return new Promise((resolve, reject) => {
        ...
        client.request(method === 'getNonce' ? 'nodeHello' : 'requestCommand', {command: params.params[0]}, (err, res) => {
            if(err) throw err
            // console.log(params[0].hash == res.result)
            resolve(JSON.parse(res.result).header.nonce.substring(16))
        })
    })
};
...
Jsonrpc.get = function (method, params, host) {
    return new Promise((resolve, reject) => {
        ...
        client.request('query', {data: params}, (err, res) => {
            if(err) throw err
