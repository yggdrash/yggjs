...
Jsonrpc.sendTX = function (method, params, host) {
    return new Promise((resolve, reject) => {
        ...
        let test = client.request(method, {tx: params[0]}, (err, res) => {
            if(err) throw err
            resolve(res.result)
        })
        console.log('tx',test)
        // console.log('txBody',test.params.tx.body)
    })
};
...
Jsonrpc.contract = function (params, host) {
    return new Promise((resolve, reject) => {
        let client = jayson.client.http(`${host}/api/contract`)
        let test = client.request('contract', {
            contractId: params.contractId ? params.contractId :"",
            method: params.method
            }, (err, res) => {
                    if(err) throw err
