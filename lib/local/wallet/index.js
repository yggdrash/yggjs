...
// var scryptsy = require('./utils/scrypt');
...
Wallet.fromV1 = function (input, password) {
  ...
  let json = (typeof input === 'undefined' ? 'undefined' : _typeof(input)) === 'object' ? input : JSON.parse(input);
  ...
  scrypt(Buffer.from(password), Buffer.from(json.Crypto.Salt, 'hex'), {
    N: kdfparams.N,
    r: kdfparams.R,
    p: kdfparams.P,
    dkLen: kdfparams.DkLen,
  ...
  let decipher = crypto.createDecipheriv('aes-128-cbc', yggUtil.sha3(derivedKey.slice(0, 16)).slice(0, 16), Buffer.from(json.Crypto.IV, 'hex'));
